...
describe('content mistral request', () => {
    it('returns basic content for basique typeCard with kanji and romanji', () => {
      ...
      expect(result).toContain('Tu dois écrire les énoncés, questions, réponses en japonais. PAS DE FRANCAIS.');
    });
    ...
    it('returns basic content for basique typeCard without kanji and romanji', () => {
      ...
      expect(result).toContain('Tu dois générer 3 cartes anki ne pas utiliser les romanji');
      expect(result).toContain('ne pas utiliser les kanji');
      expect(result).toContain('Pour un niveau de japonais de N4');
      expect(result).not.toContain('Tu dois écrire les énoncés, questions, réponses en japonais. PAS DE FRANCAIS.');
    });
    ...
    it('returns kanji content for kanji typeCard', () => {
      ...
      expect(result).toContain('Tu es fais pour faire des cartes anki pour apprendre les kanjis japonais');
    ...
    it('returns correct content for basique typeCard with japanese=false', () => {
      ...
      expect(result).toContain('Tu dois générer 4 cartes anki ne pas utiliser les romanji');
      expect(result).toContain('tu peux intégrer les kanji si il y en a');
      expect(result).not.toContain('Tu dois écrire les énoncés, questions, réponses en japonais. PAS DE FRANCAIS.');
