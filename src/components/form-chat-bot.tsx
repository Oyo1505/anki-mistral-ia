...
const SelectLevel =  dynamic(() => import('./select-level'), { ssr: false });
 ...
 const FormDataSchemaChatBot = z.object({
  name: z.string().min(1, 'Le nom est requis').max(20, 'Le nom ne doit pas dépasser 20 caractères').max(20, 'Le nom ne doit pas dépasser 20 caractères'),
  type: z.string().min(1, 'Le type d\'exercice est requis').max(300, 'Le nom ne doit pas dépasser 300 caractères'),
  ...
  return (
    ...
    <div className="w-full h-full md:h-auto border-white shadow-zinc-600 shadow-2xl rounded-md p-4 bg-white border-2">
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4 w-full">
        ...
        <SelectLevel className='w-full' register={register} levels={levels} defaultValue={formData.level} />
        ...
        {errors.level && <p className="text-red-500">Le niveau est requis</p>}
      </form>
    </div>
    )}
    </>
